...
## AnomalyCLIP Extension: Medical Report Generation & Evaluation

This repository extends BiomedCoOp with an anomaly detection + report generation pipeline:

### Features

- **AnomalyCLIP Model**: Three-path architecture (classification + memory bank anomaly detection + lesion masking)
- **Medical Report Generator**: Model outputs → structured Chinese radiology reports (4-section: findings, concept analysis, impression, recommendations)
- **CE + NLG Evaluation**: Clinical Efficacy metrics (finding-level Precision/Recall/F1) + NLG metrics (BLEU-1~4, ROUGE-L)
- **Dataset-Agnostic**: Auto-adapts classnames, modality, and findings for any medical imaging dataset
- **Multi-Dataset Support**: Works with 17 medical datasets including BUSI (breast ultrasound) and Thymoma (chest CT)

### Quick Start

```bash
# BUSI demo (breast ultrasound, already trained)
python generate_report.py --demo

# Thymoma CT pipeline
python build_thymoma_dataset.py                          # Step 1: nii → 2D slices
python train.py --config-file configs/trainers/AnomalyDetect/few_shot/thymoma.yaml \
                --dataset-config-file configs/datasets/thymoma.yaml \
                --root data --trainer AnomalyDetect_BiomedCLIP \
                --output-dir output/thymoma               # Step 2: train
python generate_report.py --config-file configs/trainers/AnomalyDetect/few_shot/thymoma.yaml \
                          --dataset-config-file configs/datasets/thymoma.yaml \
                          --model-dir output/thymoma       # Step 3: report + score
```
...
| File | Description |
|------|-------------|
| `trainers/AnomalyDetect/report_generator.py` | Model outputs → structured Chinese reports |
| `trainers/AnomalyDetect/report_scorer.py` | CE + NLG evaluation metrics |
| `generate_report.py` | Entry point: inference + report + scoring |
| `build_thymoma_dataset.py` | CT nii → 2D slice dataset builder |

### Output Structure

```
output/
├── generated_reports/    # Per-patient report text files
└── evaluation/           # Scoring data
    ├── test_gts.txt      # Reference reports
    ├── test_res.txt      # Generated reports
    ├── per_sample.json   # Per-sample details
    ├── metrics.json      # CE + NLG summary
    └── metrics.csv       # Paper-ready table row
```
